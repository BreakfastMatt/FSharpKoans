...
module ``15: Advanced techniques`` = 
    ...
    let ``01 Unit is used when there is no return value for a function``() = 
        ...
        sendData "data" |> should equal () // ... don't overthink this one! (I almost did :P)
...
*)
    ...
    *)
    ...
    let ``07 Partially specifying arguments (Part 3).`` () =
        ...
        let f animal noise = animal + " says " + noise 
        let cows = function //<-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.
        |a -> f "cow" ( a + ", de gozaru")
        |_ -> failwith "Error (test 7)"
